namespace OOP_LEGASPI_CPE201
{
    partial class FoodOrderingPrint
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            printDisplayListbox = new ListBox();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Algerian", 20.25F, FontStyle.Bold | FontStyle.Italic, GraphicsUnit.Point, 0);
            label1.Location = new Point(60, 9);
            label1.Name = "label1";
            label1.Size = new Size(354, 30);
            label1.TabIndex = 87;
            label1.Text = "Pizza ni Karlo Legaspi";
            // 
            // printDisplayListbox
            // 
            printDisplayListbox.FormattingEnabled = true;
            printDisplayListbox.ItemHeight = 15;
            printDisplayListbox.Location = new Point(60, 42);
            printDisplayListbox.Name = "printDisplayListbox";
            printDisplayListbox.Size = new Size(354, 454);
            printDisplayListbox.TabIndex = 88;
            // 
            // FoodOrderingPrint
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(886, 628);
            Controls.Add(printDisplayListbox);
            Controls.Add(label1);
            Name = "FoodOrderingPrint";
            Text = "FoodOrderingPrint";
            Load += FoodOrderingPrint_Load;
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label label1;
        public ListBox printDisplayListbox;
    }
}